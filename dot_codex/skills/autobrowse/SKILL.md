...
Prefer the `browse` CLI when it can reach the site and complete the task. Use Browser, Chrome automation, or Computer Use only when `browse` is blocked by missing capabilities, unavailable authentication, debugging needs, or explicit user direction.

Run `browse` headless by default. Do not pass `--headed` in normal attempts or finished site skills; use `--headed` only for debugging a broken selector, visual layout issue, or login flow that cannot be understood from snapshots.
...
2. Run one headless browser attempt with `browse` in local mode unless the user approved remote mode.
...
```bash
browse stop
# Headless by default. Do not add --headed unless debugging.
...
- Do not put `--headed` in a finished site skill unless the site genuinely cannot run headless and that limitation is proven in traces.
