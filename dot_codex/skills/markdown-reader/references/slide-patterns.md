...
CSS patterns, JS engine, slide type layouts, transitions, navigation chrome, and the shared deck palette for self-contained HTML slide presentations. All slides are viewport-fit (100dvh), single-file, same philosophy as scrollable pages.
...
**Before generating**, also read `./css-patterns.md` for shared patterns (Mermaid zoom controls, overflow protection, depth tiers, status badges) and `./libraries.md` for Mermaid theming, Chart.js, and typography. Those patterns apply to slides too — this file adds slide-specific patterns on top.
...
1. **Title slide** (always): background image that sets the deck's visual tone. Match the topic and the deck palette. Use `--aspect-ratio 16:9`.
2. **Full-bleed slide** (always if deck has one): immersive background for the deck's visual anchor moment. Style should match the deck's background treatment — photo-realistic, abstract/geometric, or terminal aesthetic.
...
surf gemini "descriptive prompt matching deck palette" --generate-image /tmp/mdr-slide-title.png --aspect-ratio 16:9

# Base64 encode for self-containment (macOS)
TITLE_IMG=$(base64 -i /tmp/mdr-slide-title.png)
# Linux: TITLE_IMG=$(base64 -w 0 /tmp/mdr-slide-title.png)

# Embed in the slide
# <div class="slide__bg" style="background-image:url('data:image/png;base64,${TITLE_IMG}')"></div>

# Clean up
rm /tmp/mdr-slide-title.png
```

**Prompt craft for slides:** Be specific about style, dominant colors, and mood, and pull the colors from the deck palette variables. Examples:
- Night: "abstract composition on near-black (#131210), warm sand accent light (#d9b781), cinematic depth of field"
- Night: "dark abstract circuit board pattern, faint warm traces on near-black (#131210), minimal, technical"
- Paper: "warm cream textured paper (#faf7f0) with restrained umber geometric accents (#7a4a13), confident modern design"
...
Decks use the same approved palette as every other page this skill produces: night is the
default, paper is the only alternate, the OS setting is never followed, and there are no
web-font downloads. Slides keep their own type scale and nav chrome — only the palette and
font stacks are shared.

```css
:root {
  --font-body: -apple-system-ui-serif, ui-serif, "Iowan Old Style", Palatino, Georgia, serif;
...
html[data-theme="night"] {
  ...
  --border: #33302a;
  --border-bright: #4a463e;
  --text: #e7e2d7;
  --text-dim: #aaa397;
  --accent: #d9b781;
  --accent-dim: rgba(217, 183, 129, 0.12);
  --code-bg: #0e0d0b;
  --code-text: #e7e2d7;
  color-scheme: dark;
}
...
html[data-theme="paper"] {
  ...
  --text-dim: #6d665a;
  --accent: #7a4a13;
  --accent-dim: rgba(122, 74, 19, 0.08);
  --code-bg: #231f1a;
  --code-text: #e7e2d7;
  color-scheme: light;
}
...
### Making decks feel distinct

Variety comes from treatment and composition, not from swapping palettes or typefaces:

- **Background treatment.** Pick one per deck and hold it: a radial accent glow anchored to a
  corner, a faint dot grid, clean flat surfaces with visible grid lines, or per-slide gradient
  variation from the Per-Slide Background Variation pattern.
- **Decoration.** Corner marks, divider numbers, and quote marks in `--accent` — or none at all
  for an austere deck.
- **Title treatment.** Display serif at maximum scale, or weight-400 mono for a developer-native
  feel. Section numbers at weight 200 read as decoration rather than content.
- **Density.** An analytical deck runs tight geometric layouts and data slides; a narrative deck
  runs fewer objects at larger scale.
