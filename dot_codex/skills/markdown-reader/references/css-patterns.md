...
Reusable patterns for layout, connectors, theming, and visual effects in self-contained HTML
diagram, dashboard, and deck pages.

**Scope.** These patterns are for standalone visual pages you build by hand. Reading pages are
not built by hand: author Markdown and render it with `scripts/render.py`, which owns the prose
typography, Contents dialog, A-/A+ control, and Paper/Night toggle.

## Theme Setup

Use the approved skill palette. Night is the default, paper is the only alternate, and the OS
setting is never followed. Switch by setting `data-theme="paper"` on `<html>`.

```css
:root {
  /* Native stacks only. No web-font downloads. */
...
html[data-theme="night"] {
  ...
  /* Semantic accents for diagram elements, over the night base */
  --node-a: #22d3ee;
...
```html
<div class="mdr-card" style="--i: 0">First</div>
<div class="connector">...</div>
<div class="mdr-card" style="--i: 1">Second</div>
<div class="connector">...</div>
<div class="mdr-card" style="--i: 2">Third</div>
```
...
Patterns for the explanatory text inside a standalone dashboard, diagram, or deck page. A page
whose job is sustained reading is not built from these: author Markdown and render it with
`scripts/render.py`.

### Body Text Settings

Match the reader's measure and rhythm so prose reads the same wherever it appears.
...
Standalone pages may offer the same Paper/Night toggle the reader uses. Honour a saved explicit
choice; never follow the OS setting and never pick a random initial theme. Palette values come
from "Theme Setup" above.

```javascript
// Apply a saved choice before first paint so there is no flash.
(function () {
  try {
    var t = localStorage.getItem('theme');
    if (t === 'paper' || t === 'night') document.documentElement.setAttribute('data-theme', t);
  } catch (e) {}
})();

function toggleTheme() {
  const next = document.documentElement.getAttribute('data-theme') === 'paper' ? 'night' : 'paper';
...
[data-theme="paper"] .theme-toggle__moon { display: none; }
